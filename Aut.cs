        ...
        switch(Control.YCursorPos -1)
        {
            ...
            case 3:
                ...
                foreach (AutObject checuser in checusers)
                {
                    ...
                    {
                        role=checuser.role;
                        List<EmployeObject> employees = Sterealizer.desteruser<List<EmployeObject>>(Startname.Employee);
                        foreach (EmployeObject employee in employees)
                            if(checuser.id == employee.iduser)
                            username = employee.surname+" "+employee.name+" "+employee.patronymic;
                        return false;
                    }
